...
public class SpreadingTile extends TileEntity implements ITickableTileEntity {
    ...
    public void spreadRoutine(boolean isSpreadingRandomly)
    {
        ...
        if (isSpreadingComplete || SculkHorde.gravemind.getGravemindMemory().getSculkAccumulatedMass() <= 0)
        {
            SculkHorde.infestationConversionTable.convertToDormant((ServerWorld) this.level, this.getBlockPos());
            return;
        }
        ...
        if (isSpreadingRandomly)
        {
            ...
            for (int spreadAttemptsThisTick = 0;  spreadAttemptsThisTick < allowedSpreadAttemptsPerTick && spreadAttempts < getMaxSpreadAttempts(); spreadAttemptsThisTick++)
            {
                ...
                SculkHorde.infestationConversionTable.addToConversionQueue(BlockAlgorithms.getRandomNeighbor((ServerWorld) this.level, this.getBlockPos()), getMaxSpreadAttempts() - 1, true, false, false);

                if(SculkHorde.gravemind.getGravemindMemory().isInRangeOfNode(this.getBlockPos(), 128))
                {
                    spreadAttempts +=1;
                }
                else
        ...
        else if (!isSpreadingRandomly)
        {
            ...
            {
                //Attempt to spread to this position
                SculkHorde.infestationConversionTable.addToConversionQueue(allNeighbors.get(i), getMaxSpreadAttempts() - 1, true, false, false);
            }
            isSpreadingComplete = true;
        }
